...
class PinSchema(Schema):
    ...
    mode = fields.Str(required=True, validate=lambda x: x in ['INPUT', 'OUTPUT'], metadata={"description": "Pin mode: INPUT, OUTPUT"})
    state = fields.Str(validate=lambda x: x in ['HIGH', 'LOW'], metadata={"description": "Pin state: HIGH, LOW"})
    pull = fields.Str(validate=lambda x: x in ['PULL_UP', 'PULL_DOWN', 'NONE'], metadata={"description": "Pin pull: PULL_UP, PULL_DOWN, NONE"})
...
class DeviceSchema(Schema):
    ...
    device_type = fields.Str(validate=lambda x: x in ['SENSOR', 'ACTUATOR'], metadata={"description": "Type of the device: SENSOR, ACTUATOR"})
    ...
    status = fields.Str(validate=lambda x: x in ['ACTIVE', 'INACTIVE', 'MAINTENANCE', 'RETIRED'], metadata={"description": "Status of the device: ACTIVE, INACTIVE, MAINTENANCE, RETIRED"})
